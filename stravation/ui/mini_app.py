...
from scripts.backfill_strava import run as backfill_strava_run
from scripts.push_to_sheets import push_missing
...
from stravation.features.calendar_sync import sync_gcal_with_notion
...
class PlanTab(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkButton(
            header, text="🔁 Sync GCal ← Notion", width=180,
            command=self._sync_full
    ...
    def _sync_full(self):
        """Synchronise entièrement le calendrier Google avec Notion :
        - crée les séances manquantes
        - met à jour les séances modifiées
        - supprime celles effacées de Notion
        """
        def job():
            try:
                stats = sync_gcal_with_notion()
                msg = (
                    "Synchronisation complète effectuée :\n\n"
                    f"• Créées : {stats.get('created', 0)}\n"
                    f"• Mises à jour : {stats.get('updated', 0)}\n"
                    f"• Supprimées : {stats.get('deleted', 0)}"
                )
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # ✅ Nouveau bouton : Push Sheets
        self.btn_push_sheets = ctk.CTkButton(
            header, text="📤 Push Sheets",
            command=self._push_sheets, height=36, corner_radius=12
        )
        self.btn_push_sheets.pack(side="left", padx=8)
    ...
    def _sync_activities(self):
        # Texte de statut + bouton
        self._set_status("Synchronisation des activités en cours…")
        self.btn_sync_act.configure(text="Sync en cours…")

        def _call():
            # 1) Strava -> Notion (ce que tu faisais déjà)
            sync_strava_to_notion()

            # 2) Backfill Strava -> Notion + Google Sheets
            #    seulement sur les X derniers jours pour aller vite
            days_back = 15  # tu peux changer ici (3, 5, 10…)
            since_date = p.now().subtract(days=days_back).to_date_string()

            backfill_strava_run(
                limit=0,
                only_missing=True,
                force=True,  # on recalcule TRIMP / FC / calories même si déjà présents
                since=since_date,
                dry_run=False,
                sleep=0.15,
            )

        # On lance tout ça en tâche de fond
        self._run_bg(
            _call,
            on_ok="Activités sync Notion + Sheets",
            on_err_prefix="[Sync activités] Erreur",
            btn=self.btn_sync_act,
        )

    def _push_sheets(self):
        """Push Notion -> Google Sheets sur les X derniers jours via Strava ID."""
        self._set_status("Push vers Google Sheets…")
        self.btn_push_sheets.configure(text="Push en cours…")

        def _call():
            # Comparaison Notion vs Sheets sur les 15 derniers jours, via Strava ID
            push_missing(days=15)

        self._run_bg(
            _call,
            on_ok="Push Sheets OK",
            on_err_prefix="[Push Sheets] Erreur",
            btn=self.btn_push_sheets,
        )
